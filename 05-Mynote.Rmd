...
$S_{3} = \left\{ i,\ \ \sigma_{1},\ \ \sigma_{2},\ \ \sigma_{3},\ \ \tau_{1},\ \ \tau_{2},\ \ \tau_{3} \right\}$
and ${|S}_{n}| = n!$

$S_{3}$ -- symmetric group of 3 elements

${(\ S}_{3},\  \circ \ )$ is a group.

$\sigma_{1^{0}}\sigma_{2} = \begin{pmatrix}
1 & 2 & 3 \\
2 & 3 & 1 \\
...
```
  ...
  ----------------------------------------------------------------------------------------------------------
  $$\circ$$        $$i$$      $$\sigma_{1}$$   $$\sigma_{2}$$   $$\tau_{1}$$   $$\tau_{1}$$   $$\tau_{1}$$
  ---------------- ---------- ---------------- ---------------- -------------- -------------- --------------
  $$i$$                                                                                       
...
```{theorem}
If $\sim$ is an equivalence relation on $S$ then
...
$|H|$ $1 \Rightarrow \left\{ \mathbb{e} \right\}$
$p \Rightarrow G$
```
...
Let $n 1$ be a fixed integer. $a,b\mathbb{\in Z}$. Define
...
$a.b = (ns + c)\left( {nt}{+ d} \right) = n^{2}st + nsd + ntc + c.d$
...
$\overline{a}.\overline{1} = \overline{a.1} = \overline{a} = \overline{1.a} = \overline{1}.\overline{a}$
...
$\mathbb{Z}_{n} \smallsetminus \left\{ \overline{0} \right\}$ that have
an inverse in
...
$0\left( a^{t} \right) = \frac{n}{\gcd(t,n)}$
```
...
Now
$a^{r} = a^{m - nq} = a^{m}.a^{- nq} = \mathbb{e}.\left( a^{n} \right)^{- q} = \mathbb{e}.\mathbb{e}^{- q} = \mathbb{e}$

Since $n$ is the smallest positive integer such that
$a^{n} = \mathbb{e}$ and $a^{r} = \mathbb{e}$, it follows that $r = 0$.

Thus $m = nq$.

This implies that $n$ divides $m$.

ii. Let $0\left( a^{t} \right) = k$.

Then $a^{tk} = e.$

By (i.) $n$ divides $kt$.

Thus $\exists r\mathbb{\in Z}$ such that $kt = nr$.

Let $\gcd(t,n) = d$.

Then $\exists u,v\mathbb{\in Z}$ such that $t = du,\ \ n = \mathbb{d}v$
and $\gcd(u,v) = 1$
...
Let $G$ be a group. If $a \in G$ and $O(a) = m,$ then

$A = \left\{ \mathbb{e},a,a^{2},\ .\ .\ .,a^{m - 1}\  \right\}$
...
Take $A = \left\{ \mathbb{e},a,a^{2},\ .\ .\ .,a^{m - 1}\  \right\}$

We have seen that $A \subset G$ and $|A| = m = O(a)$

By Lagrange's theorem,

$|A|\ |\ |G| \Rightarrow O\left. \ (a) \right|\ |G|$

Since $O\left. \ (a) \right|\ |G|$, we have

$n = |G| = kO(a)$

$a^{n} = a^{kO(a)} = \left( a^{O(a)} \right)^{k} = \mathbb{e}^{k} = \mathbb{e}$
